...
2. In `src/lib/db/queries.ts`, add two new exported async functions:
  ...
  <verify>
    Run `npx tsc --noEmit` — no type errors. Visually confirm emailClicks table has correct columns, FK, and indexes. Confirm both query functions use Drizzle builder (no raw SQL) and handle errors gracefully.
  </verify>
  <done>
    email_clicks table defined in schema.ts with uuid PK, shopId, messageLogId FK, linkUrl, clickedAt. Two query functions (recordEmailOpen, recordEmailClick) exist in queries.ts, both idempotent and non-throwing.
  </done>
</task>
