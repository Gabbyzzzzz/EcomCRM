...
Replace the current Message History table's "Status", "Opened", and "Clicked" columns with a single "Engagement" column approach using status icons from lucide-react. Update the table to match TRACK-03 requirements:

1. **Import lucide-react icons** at the top of the file:
   ```typescript
   import { CheckIcon, EyeIcon, Link2Icon } from 'lucide-react'
   ```
   (lucide-react is already installed in this project — no need to add it.)

2. Keep the existing columns: Date, Subject, Automation, Status.
3. Replace the separate "Opened" and "Clicked" text columns with icon-based display as a dedicated "Engagement" column.
4. The status display logic with icons:
   - If `clickedAt` is set: show Link2Icon + "Clicked" text with blue color, and the clickedAt timestamp
   - Else if `openedAt` is set: show EyeIcon + "Opened" text with green color, and the openedAt timestamp
   - Else if status is 'sent': show CheckIcon + "Sent" text with muted color, and sentAt timestamp
...
4. The `stats` fetch should run concurrently with the automation query. Restructure:
  ...
  <done>
    Customer profile message history uses lucide-react status icons (Link2Icon for Clicked, EyeIcon for Opened, CheckIcon for Sent) with text labels and timestamps in a single Engagement column. Automation detail page shows per-flow open rate and click rate with MPP caveat. getAutomationEmailStats query function aggregates from message_logs.
  </done>
</task>
