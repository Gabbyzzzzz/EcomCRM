...
must_haves:
  ...
  artifacts:
    ...
    - path: "README.md"
      provides: "Email Sending Setup section with DNS records checklist"
  key_links:
...
Output: 5 email templates, send wrapper, schema extensions, unsubscribe token utility, DNS setup docs in README.md
</objective>
...
<task type="auto">
  <name>Task 1: Schema extensions + env vars + unsubscribe token utility + DNS docs</name>
  <files>
    src/lib/db/schema.ts
    src/lib/env.ts
    src/lib/email/unsubscribe.ts
...
**Add "Email Sending Setup" section to `README.md`:**

Add a `## Email Sending Setup` section to the project README.md with the following content:
- Brief explanation that EcomCRM uses Resend for transactional email delivery
- **Domain Verification** subsection: Link to Resend domain verification docs (`https://resend.com/docs/dashboard/domains/introduction`), recommend using a `marketing.` subdomain
- **DNS Records Checklist:**
  - SPF (TXT record): `v=spf1 include:amazonses.com ~all`
  - DKIM: Automatically configured by Resend during domain verification
  - DMARC (TXT record): `v=DMARC1; p=none; rua=mailto:dmarc@your-domain.com`
- **Env Vars:** Reference the `RESEND_FROM_NAME`, `RESEND_FROM_EMAIL`, and `RESEND_REPLY_TO` env vars and what they control
- Note that DNS propagation can take up to 48 hours
...
Run `npx drizzle-kit generate` to produce the migration file for the new column and table.
  ...
  <verify>
    ...
    5. README.md contains an "Email Sending Setup" section with DNS records checklist
  </verify>
  <done>
    ...
    - README.md has "Email Sending Setup" section with SPF/DKIM/DMARC DNS records checklist and link to Resend domain verification docs
...
interface SendMarketingEmailParams {
  ...
  subject: string
  templateFactory: (unsubscribeUrl: string) => ReactElement  // Factory receives the generated unsubscribeUrl
...
**IMPORTANT: Template factory pattern.** The `template` parameter is a factory function `(unsubscribeUrl: string) => ReactElement`, NOT a pre-instantiated ReactElement. This ensures callers cannot forget to pass the unsubscribe URL to the template body. The wrapper calls `buildUnsubscribeUrl(customerInternalId, shopId)` once and passes the resulting URL to BOTH the factory function (for the template footer link) AND the `List-Unsubscribe` header. This guarantees the URL in the header and the URL in the email body are always identical and always present.
...
await sendMarketingEmail({
  ...
  subject: 'Welcome!',
  templateFactory: (unsubscribeUrl) => <WelcomeEmail storeName="MyStore" customerName="Jane" unsubscribeUrl={unsubscribeUrl} />,
...
5. Build the unsubscribe URL using `buildUnsubscribeUrl(customerInternalId, shopId)`.
6. Call the `templateFactory(unsubscribeUrl)` to get the ReactElement, then render to HTML string using `render(element)` from `@react-email/render`.
...
Export `sendMarketingEmail` as a named export.
  ...
  <verify>
    ...
    4. `sendMarketingEmail` signature accepts `templateFactory: (unsubscribeUrl: string) => ReactElement` (not a bare ReactElement)
    5. All imports resolve correctly (no missing modules)
    6. `npm run build` succeeds
  </verify>
  <done>
    - 5 React Email templates exist in src/emails/ with consistent branding, mobile-responsive layout, and unsubscribe footer
    - sendMarketingEmail accepts a templateFactory function that receives the generated unsubscribeUrl, ensuring callers cannot omit the URL from the template body
    - sendMarketingEmail checks marketing_opted_out, checks suppression table, calls templateFactory with the unsubscribeUrl, renders template, sends via Resend with List-Unsubscribe + List-Unsubscribe-Post headers and idempotency key
    - The same unsubscribeUrl is used in both the List-Unsubscribe header and passed to the template factory
...
6. `sendMarketingEmail` uses templateFactory pattern (not pre-instantiated ReactElement)
7. README.md contains "Email Sending Setup" section with DNS records checklist
</verification>
...
- DNS setup documented in README.md per locked decision
</success_criteria>
