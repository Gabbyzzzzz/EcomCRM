...
3. **Enhance PATCH endpoint to return the updated row** (`src/app/api/automations/[id]/route.ts`):
   ...
   - After calling `setAutomationEnabled(id, parsed.data.enabled)`, query the automation row back from the DB to confirm the write succeeded and return it in the response.
   - Use an inline Drizzle query to fetch the updated row: `const [updatedRow] = await db.select().from(automations).where(and(eq(automations.id, id), eq(automations.shopId, shopId))).limit(1)` where `shopId` is derived from `process.env.SHOPIFY_STORE_URL` hostname (e.g., `new URL(process.env.SHOPIFY_STORE_URL!).hostname`). Import `db` from `@/lib/db`, `automations` from `@/lib/db/schema`, and `eq`, `and` from `drizzle-orm`.
   - Return: `NextResponse.json({ ok: true, automation: { id: updatedRow.id, enabled: updatedRow.enabled } })`.
   - If `updatedRow` is undefined (row not found after update), return 404.
   - Add a GET handler to this route that returns the full automation row (for use by other components that need to fetch fresh server state). Use the same inline query pattern: derive shopId from env, query by id + shopId, return 404 if not found.
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <action>
Human verification checkpoint. What was built: Badge text fix (Active/Inactive instead of Disabled) on both list and detail pages, plus PATCH endpoint now returns the updated automation row with confirmed DB state.
