...
must_haves:
  truths:
    ...
    - "The days_since_order trigger exercises TWO flows — Repurchase Prompt and Win-Back Campaign — confirmed by cron wiring audit in Task 1 and direct action execution in Task 2"
...
6. **Confirm days_since_order cron wiring**: Trace the `checkDaysSinceOrder` Inngest function to confirm it calls `fetchEnabledAutomationsByTrigger(shopId, 'days_since_order')` and iterates over eligible customers. Document in a `[pipeline]` log comment that the cron path was audited and confirmed correct (or fix if broken). This audit plus the direct execution test in Task 2 together verify the full days_since_order path.

7. **Add a normalization smoke-test comment block**: After writing any normalization code, add a comment block with a sample REST webhook payload and the expected normalized output, so future developers (and the executor) can visually confirm the mapping is correct without a live Shopify webhook. Example:
   ```
   // Normalization smoke test:
   // Input (REST): { id: 123, total_price: "99.00", customer: { id: 456 }, ... }
   // Output (normalized): { id: "gid://shopify/Order/123", totalPriceSet: { shopMoney: { amount: "99.00" } }, customer: { id: "gid://shopify/Customer/456" }, ... }
   ```
...
2. Check that all Inngest event names match between senders and receivers by grepping for event name strings.
3. If a normalization function was added, verify it handles the sample payload from the smoke-test comment correctly: write a quick inline assertion at the bottom of the file (guarded by `if (process.env.NODE_ENV === 'test')`) that calls the normalizer with the sample REST payload and checks the output fields match expectations, OR manually trace the field mapping for the sample and confirm correctness in the summary.
  </verify>
  <done>All 5 automation flows have correct wiring from trigger event through to email send, with no type mismatches or payload format issues. TypeScript compiles cleanly. The days_since_order cron path is audited and confirmed correct. Any normalization logic has a documented sample mapping.</done>
</task>
...
4. Based on triggerType, send the corresponding Inngest event:
   ...
   - `days_since_order`: This is a cron job, not event-driven. Instead, directly import `fetchEnabledAutomationsByTrigger` from `@/lib/automation/engine` and `executeEmailAction` from `@/lib/automation/actions`. Call `fetchEnabledAutomationsByTrigger(shopId, 'days_since_order')` and for each enabled automation, call `executeEmailAction` directly for the given customer (skip the cron scan logic). This gives immediate feedback without needing the Inngest cron to run. Note: this exercises the same action execution path the cron uses — the cron wiring itself is confirmed by audit in Task 1.
...
5. Return `{ ok: true, triggerType, eventSent: true }` on success (or `{ ok: true, triggerType, directExecution: true }` for days_since_order).
...
Use TypeScript strict mode, no `any`. Import inngest client from `@/inngest/client`, db and schema from `@/lib/db`, automation engine from `@/lib/automation/engine`, and actions from `@/lib/automation/actions`.
  </action>
  <verify>
Run `npx tsc --noEmit` to verify no type errors. Then start the dev server and Inngest dev server, POST to `/api/automations/test-trigger` with `{ "triggerType": "first_order", "customerId": "<real-customer-uuid>" }` and verify the Inngest dashboard shows the event was received and the function was invoked.
  ...
  <done>POST /api/automations/test-trigger accepts all 4 trigger types, sends the correct Inngest events (or directly executes for days_since_order), and returns a success response. The endpoint is guarded against production use. The 4 trigger types cover all 5 preset flows: first_order covers Welcome, segment_change covers VIP, days_since_order covers both Repurchase Prompt and Win-Back Campaign, and cart_abandoned covers Abandoned Cart.</done>
</task>
...
6. Any payload normalization logic has a documented sample mapping and the mapping is confirmed correct
</verification>

<success_criteria>
- The full automation pipeline compiles without type errors and all event wiring is audited and corrected
- A test-trigger endpoint exists for manually firing all 5 automation flows (4 trigger types covering 5 flows: days_since_order covers both Repurchase Prompt and Win-Back Campaign)
- After triggering, message_logs rows confirm email send attempts were made
- The days_since_order cron wiring is audited and confirmed correct in Task 1, and the action execution path is verified via direct invocation in Task 2
</success_criteria>
