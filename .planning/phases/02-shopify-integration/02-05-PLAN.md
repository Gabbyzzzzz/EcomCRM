...
must_haves:
  truths:
    ...
    - "All .planning/codebase/ docs that listed SHOPIFY_ACCESS_TOKEN now list SHOPIFY_CLIENT_ID + SHOPIFY_CLIENT_SECRET (including CONCERNS.md line 33)"
  artifacts:
    - path: "CLAUDE.md"
...
<task type="auto">
  <name>Task 2: Update codebase map docs (STACK.md, INTEGRATIONS.md, STRUCTURE.md, CONCERNS.md)</name>
  <files>.planning/codebase/STACK.md, .planning/codebase/INTEGRATIONS.md, .planning/codebase/STRUCTURE.md, .planning/codebase/CONCERNS.md</files>
  <action>
...
**CONCERNS.md — Security & Configuration Risks section (line 33):**

The "Secrets Management" risk bullet currently reads:
```
...
This corrects the historical snapshot: SHOPIFY_ACCESS_TOKEN was replaced by SHOPIFY_CLIENT_ID + SHOPIFY_CLIENT_SECRET in the OAuth refactor (commit ea8b9bb), and NEXT_PUBLIC_SUPABASE_URL was added as a required var.

Do NOT change any other concern entries, headings, recommendations, or non-env-var content in these files.
  </action>
  <verify>
...
`grep "SHOPIFY_ACCESS_TOKEN" /Users/zhangjiabei/Desktop/ecomcrm/.planning/codebase/CONCERNS.md` — must return zero results.
...
`grep "SHOPIFY_CLIENT_ID" /Users/zhangjiabei/Desktop/ecomcrm/.planning/codebase/CONCERNS.md` — must return at least 1 result.
  </verify>
  <done>
- STACK.md, INTEGRATIONS.md, STRUCTURE.md, and CONCERNS.md contain zero references to SHOPIFY_ACCESS_TOKEN.
- All four files reference SHOPIFY_CLIENT_ID and SHOPIFY_CLIENT_SECRET in their env var listings.
...
<success_criteria>
- Zero `SHOPIFY_ACCESS_TOKEN` references in CLAUDE.md, REQUIREMENTS.md, STACK.md, INTEGRATIONS.md, STRUCTURE.md, and CONCERNS.md.
- All six files reference SHOPIFY_CLIENT_ID and SHOPIFY_CLIENT_SECRET where auth env vars are listed.
