...
- `shopifyUpdatedAt`: timestamp('shopify_updated_at', { withTimezone: true }) — for last-write-wins upsert on out-of-order webhooks (needed by 02-02 incremental handlers)
...
Export all new tables and enums with `export const` at module top level (critical — drizzle-kit skips non-exported items).
  ...
  <verify>Run `npx drizzle-kit generate` — it should produce a new migration file (0001_*.sql) with ALTER TABLE statements for new columns and CREATE TABLE for new tables. Check the SQL file contains: is_historical, deleted_at, shopify_updated_at (on BOTH customers AND orders tables), sync_logs, webhook_deliveries.</verify>
  <done>Schema has syncLogs, webhookDeliveries tables, orders.isHistorical, orders.shopifyUpdatedAt, orders.shopifyCreatedAt, customers.deletedAt, customers.shopifyUpdatedAt columns. Migration SQL generated successfully.</done>
</task>
...
- orders table has shopifyUpdatedAt column (needed by 02-02 for last-write-wins on order upserts)
</verification>

<success_criteria>
- Migration SQL exists with new tables (sync_logs, webhook_deliveries) and new columns (is_historical, deleted_at, shopify_updated_at on BOTH customers AND orders)
