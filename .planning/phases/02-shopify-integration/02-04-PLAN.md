...
must_haves:
  truths:
    ...
    - "When all Inngest retries are exhausted for a webhook, the webhookDeliveries row is updated to status='dead_letter'"
  ...
  artifacts:
    ...
    - path: "src/lib/db/queries.ts"
      provides: "True last-write-wins using lte timestamp comparison in setWhere"
  ...
  key_links:
    ...
    - from: "src/inngest/functions.ts (onFailure handler)"
      to: "src/lib/db/queries.ts (recordWebhookDelivery)"
      via: "recordWebhookDelivery with status='dead_letter'"
      pattern: "dead_letter"
...
export const processShopifyWebhookFailure = inngest.createFunction(
  ...
  async ({ event }) => {
    ...
    await recordWebhookDelivery(shopId, webhookId, topic, 'dead_letter')
  }
)
```
...
Note on the `inngest/function.failed` event: Inngest fires this event automatically when a function exhausts all retries. The `event.data.event` contains the original event that failed. Filter by `event.data.function_id === 'process-shopify-webhook'` if needed to avoid the handler triggering for scheduledSync failures.
  </action>
  <verify>
...
- A `processShopifyWebhookFailure` Inngest function exists, listening on `inngest/function.failed`, calling `recordWebhookDelivery(..., 'dead_letter')` with shopId and webhookId from the failed event data.
...
3. `grep -n "dead_letter" /Users/zhangjiabei/Desktop/ecomcrm/src/inngest/functions.ts` — shows both the onFailure handler body and the comment on the throw
...
- onFailure dead-letter handler exists as exported Inngest function and is in the functions array (Gap 1 dead-letter fix)
