...
must_haves:
  truths:
    ...
    - "Failed full sync with a cursor can be resumed from where it left off — not from scratch"
  artifacts:
    - path: "src/lib/shopify/sync.ts"
      provides: "Bulk sync logic — start, poll/receive completion, process JSONL results, resume from cursor"
  ...
  key_links:
    ...
    - from: "src/lib/shopify/sync.ts"
      ...
      via: "upsertCustomer/upsertOrder imports for processing sync results"
      pattern: "import.*upsertCustomer|upsertOrder.*from.*db/queries"
---
...
2. **`upsertOrder(shopId: string, data: {...}, isHistorical: boolean)`**: Similar upsert on (shopId, shopifyId). Set isHistorical flag. Money: `new Decimal(totalPrice).toString()`. Add unique composite index on orders table too: `uniqueIndex('orders_shop_shopify_unique').on(table.shopId, table.shopifyId)`. Use last-write-wins on `shopifyUpdatedAt` (orders table now has this column per 02-01).
...
9. **`getFailedSyncWithCursor(shopId: string)`**: Find the most recent syncLog with status='failed' AND cursor IS NOT NULL for this shop. Return it or null. Used by startFullSync to resume from checkpoint.
...
5. **`SINGLE_ORDER_QUERY`**: Query to fetch a single order by GID — same fields as bulk.
  ...
  <verify>Run `npx drizzle-kit generate` to confirm migration generates. TypeScript compilation passes for queries.ts and shopify/queries.ts files. Verify getFailedSyncWithCursor is exported.</verify>
  <done>DB query functions handle upserts with selective merge, idempotency checks, sync log tracking, and failed-sync-with-cursor lookup. GraphQL query strings ready for use. Unique composite indexes added to schema. Migration generated.</done>
</task>
...
1. **`startFullSync(shopId: string)`**:
   - **Resume logic (per locked decision — checkpoint-based resume):** First, call `getFailedSyncWithCursor(shopId)`. If a failed syncLog with a non-null cursor is found, resume from that checkpoint: update its status back to 'running', and use the stored cursor to skip already-processed records when processing the JSONL results (pass the cursor to processFullSyncResults so it can skip lines until reaching the cursor position). If no failed sync with cursor exists, start fresh.
   - For a fresh sync: create a new syncLog entry (status: 'running')
...
2. **`processFullSyncResults(syncLogId: string, shopId: string, jsonlUrl: string, syncStartedAt: Date, resumeCursor?: string)`**:
   - Fetch the JSONL file from the URL Shopify provides
   - Parse line by line (each line is a JSON object — customer or order)
   - **If resumeCursor is provided:** skip lines until reaching the cursor position (cursor = line number or last processed Shopify GID). This implements checkpoint-based resume per user decision.
   ...
   - **Checkpoint tracking:** After every batch of records (e.g., every 100 lines), update syncLog with current cursor position (line number or last GID) and running counts. This enables resume on failure.
   - Update syncLog with final counts and status 'completed'
   - On error: update syncLog with error message, current cursor position, and status 'failed' — so next startFullSync can resume from this point
...
1. **`processShopifyWebhook`**: `inngest.createFunction({ id: 'process-shopify-webhook', retries: 4 }, { event: 'shopify/webhook.received' }, async ({ event, step }) => {...})`
   ...
   NOTE: The 'bulk_operations/finish' case is handled directly in the switch-case of processShopifyWebhook (it arrives as a webhook topic like any other). Do NOT create a separate processFullSyncCompletion Inngest function — that would be dead code since no separate 'shopify/bulk-operation.completed' event is ever emitted.
...
Import all functions from the modules and register them in the exported `functions` array. The array should contain exactly 2 functions: processShopifyWebhook and scheduledSync.
  </action>
  <verify>TypeScript compilation passes for all new files. Check that: (1) webhook route reads raw body before parsing, (2) HMAC verification uses timing-safe comparison, (3) money values use Decimal not parseFloat, (4) upsertCustomer preserves CRM fields, (5) Inngest functions array contains exactly 2 functions (processShopifyWebhook, scheduledSync — no dead processFullSyncCompletion), (6) startFullSync checks getFailedSyncWithCursor before starting fresh, (7) processFullSyncResults accepts and uses resumeCursor parameter.</verify>
  <done>Full sync pipeline works: bulk operation start -> async completion -> JSONL processing -> DB writes. Failed syncs with cursors can be resumed from their checkpoint. Webhook endpoint verifies HMAC, deduplicates on webhook ID, dispatches to Inngest. Incremental handlers upsert with selective merge and last-write-wins. Scheduled sync runs every 6 hours. No dead code — bulk_operations/finish handled in processShopifyWebhook switch-case.</done>
</task>
...
- Inngest functions array contains exactly 2 functions (no dead processFullSyncCompletion)
...
- startFullSync checks for failed sync with cursor and resumes from checkpoint
- processFullSyncResults periodically saves cursor to syncLog for checkpoint-based resume
</verification>
...
- Checkpoint-based resume: failed sync with cursor resumes from where it left off (locked decision)
</success_criteria>
