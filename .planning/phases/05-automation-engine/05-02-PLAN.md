...
<task type="auto">
  <name>Task 1: Days-since-order cron Inngest function + preset seed API</name>
  <files>
    src/inngest/functions.ts
    src/app/api/automations/seed/route.ts
  </files>
...
Logic:
1. `step.run('fetch-automations')` — call `fetchEnabledAutomationsByTrigger(shopId, 'days_since_order')`
2. For each automation, `step.run('scan-customers-{automation.id}')`:
   a. Parse `triggerConfig` as `{ days: number, segments?: string[] }`
   b. Compute `cutoffDate = new Date(Date.now() - config.days * 24 * 60 * 60 * 1000)`
   c. Query customers: `db.select({ id, lastOrderAt, segment, shopifyId }).from(customers).where(and(eq(customers.shopId, shopId), lte(customers.lastOrderAt, cutoffDate), isNotNull(customers.lastOrderAt), isNull(customers.deletedAt)))` — import `lte, isNull, isNotNull` from drizzle-orm (already imported in file for other uses)
   d. Filter by segment: `evaluateSegmentFilter(automation, customer.segment)` — import from engine
   e. For each matching customer, call `executeEmailAction({ shopId, customerId: customer.id, automationId: automation.id, emailTemplateId: automation.emailTemplateId, eventTimestamp: new Date().toISOString() })`
   f. After all customers processed, call `updateAutomationLastRun(automation.id, new Date())`
3. Log count of emails attempted
...
Note: `lte`, `isNull`, `isNotNull`, `and`, `eq` are likely already imported — check before adding. The `orders` table may already be imported as `orders` — use `ordersTable` alias if needed to avoid conflict.
...
4. `curl -X POST http://localhost:3000/api/automations/seed` (with dev server running) returns JSON with `seeded: 5`
  </verify>
  <done>checkDaysSinceOrder function exists in functions.ts, runs on `0 3 * * *` cron, scans customers by lastOrderAt cutoff with segment filter. Seed endpoint returns 5 seeded automations when called.</done>
</task>
