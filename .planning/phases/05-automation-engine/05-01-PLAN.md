...
Exports:
- `executeEmailAction(params: EmailActionParams): Promise<void>` — calls `sendMarketingEmail` with correct templateFactory based on `emailTemplateId`. Idempotency key format: `${automationId}-${customerId}-${eventTimestamp}`. Supported templateIds: `'welcome'`, `'abandoned-cart'`, `'repurchase'`, `'winback'`, `'vip'`.

  Before calling `sendMarketingEmail`, resolve the subject line using this subject map (keyed by `emailTemplateId`):
  ...
  const subject = SUBJECT_MAP[params.emailTemplateId] ?? 'A message from our store'
  ```

  Pass `subject` as the `subject` field to `sendMarketingEmail`. This is required — `sendMarketingEmail` does not have a default subject.
...
Also update the `rfm/segment.changed` event emission site inside `dailyRfmRecalculation` to include `eventTimestamp` in the payload. Find the `inngest.send` (or `step.sendEvent`) call that emits `rfm/segment.changed` and ensure it passes `eventTimestamp: new Date().toISOString()` in the data object. This timestamp must be stable for a given recalculation run — generate it ONCE before the customer loop and reuse it for all events emitted in that batch:

```ts
const recalcTimestamp = new Date().toISOString()
// ... inside customer loop:
await inngest.send({
  name: 'rfm/segment.changed',
  data: { shopId, customerId, oldSegment, newSegment, eventTimestamp: recalcTimestamp }
})
```
...
**CRITICAL — idempotency key stability:** Read `eventTimestamp` from `event.data.eventTimestamp` (passed by the emitter in `dailyRfmRecalculation`). Do NOT call `new Date().toISOString()` inside the function body — on Inngest retry this generates a new timestamp which breaks the idempotency key in `executeEmailAction`, allowing duplicate sends.
...
inngest.createFunction(
  ...
  async ({ event, step }) => {
    const { shopId, customerId, newSegment, eventTimestamp } = event.data
...
In the `orders/create` webhook handler, before emitting `automation/first_order`, verify the order is not historical. Webhook orders are always `isHistorical=false` (per comment in existing code), so no additional guard is needed beyond only emitting for `orders/create` topic (not `orders/updated`). The orderCount check guards against repeat triggers: only emit when `customerRow.orderCount === 1` after counter update.
  ...
  <verify>Run `npx tsc --noEmit` — zero errors. Run `grep -n "process-first-order\|process-segment-change\|process-cart-abandoned" src/inngest/functions.ts` — all three IDs present. Run `grep -n "event\.data\.eventTimestamp" src/inngest/functions.ts` — must show usage in processSegmentChange (not new Date().toISOString()). Run `grep -n "eventTimestamp" src/inngest/functions.ts` — must show eventTimestamp passed in rfm/segment.changed emit inside dailyRfmRecalculation.</verify>
  <done>Three new Inngest functions exist in functions.ts and are in the exported functions array. processShopifyWebhook emits automation/first_order for first-time orders. processSegmentChange reads eventTimestamp from event.data (not generated locally). dailyRfmRecalculation passes eventTimestamp in rfm/segment.changed payload. TypeScript strict mode passes with no errors.</done>
</task>
...
7. `grep -n "SUBJECT_MAP" src/lib/automation/actions.ts` shows the subject map with all 5 template keys
8. `grep -n "event\.data\.eventTimestamp" src/inngest/functions.ts` shows usage in processSegmentChange — no local new Date() call for idempotency key
9. `grep -n "recalcTimestamp\|eventTimestamp" src/inngest/functions.ts` shows eventTimestamp passed in rfm/segment.changed emit
</verification>
...
- actions.ts contains SUBJECT_MAP with all 5 template subjects; subject is passed to sendMarketingEmail
...
- processSegmentChange reads eventTimestamp from event.data.eventTimestamp (not generated locally)
- dailyRfmRecalculation passes eventTimestamp in rfm/segment.changed event payload
